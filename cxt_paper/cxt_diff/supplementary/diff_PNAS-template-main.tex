...
%DIF ADD PNAS-template-main.tex   Wed Mar 11 14:27:15 2026
...
%DIF -------
\newcommand{\singer}{\texttt{Singer}} %DIF > 
%DIF < \newcommand{\smcpp}{\texttt{SMC++}\xspace}
\newcommand{\smcpp}{\texttt{SMC++}} %DIF > 
%DIF < \newcommand{\logspace}{\text{log-space}\xspace}
\newcommand{\logspace}{\text{log-space}} %DIF > 
%DIF < \newcommand{\model}{}
 %DIF > 
\title{\DIFaddbegin \DIFadd{Coalescence and Translation: A Language Model for Population Genetics}\DIFaddend } %DIF > 
\author{\DIFaddbegin \DIFadd{Kevin Korfmann, Nathaniel S. Pope, Melinda Meleghy, Aur\'elien Tellier, and Andrew D. Kern}\DIFaddend } %DIF > 
...
\DIFaddend Hyperparameter specifications of our narrow and broad model.
...
\DIFdelFL{and \smcpp reflecting training on a single demographic regime (constant size) versus a fluctuating demographic history (sawtooth), respectively.
    }}
    %DIFAUXCMD
...
\DIFdelendFL \DIFaddbeginFL \caption{
    ...
    \DIFaddFL{\smcpp              }& \DIFaddFL{0.8685          }& \DIFaddFL{1.7919 }\\
    \bottomrule
    \end{tabular}
\end{table}
...
\caption{\DIFaddFL{The density of mutations and recombination events across perturbations of the Zigzag }\stdpopsim \DIFaddFL{model, for the simulations shown in Figure~\ref{fig:correction-heatmap}.}}
...
\caption{\DIFaddFL{$\cxtkit$s broad model fine-tuned with a low-parameter adapter to account for small sample sizes. On the left, $\cxtkit$ is demonstrated on a constant demography, while on the right the sawtooth scenario is presented.}}
\label{fig:adapter}
\end{figure}
...
\caption{\DIFaddFL{$\cxtkit$s broad model (left) on }\stdpopsim \DIFaddFL{AnoGam parameterization at window resolution 2 Kb, in the middle at 0.2 Kb and after fine-tuning on large }\stdpopsim \DIFaddFL{species larger than 100 k $N_e$.}}
...
\caption{
\DIFaddFL{The approximate posterior distributions sampled from by }\cxtkit \DIFaddFL{give well-calibrated estimates of uncertainty for predicted TMRCAs, across a range of scenarios. Top row: the proportion of true TMRCAs that fall within credibility intervals of a given width (e.g. a given proportion of posterior mass), as a function of position in the 1Mb prediction frame. Credibility intervals are calculated from quantiles of 100 samples from the approximate posterior for each pivot pair, and the proportion covering the true TMRCAs are calculated over 1000 independent pivot pairs (e.g. from distinct genealogical simulations). The expected proportions (for an exact posterior) are shown as dashed lines. Empirically, the approximate posteriors are asymptotically well-calibrated, although slightly over-concentrated for larger interval widths (likely due to the time discretization used by }\cxtkit\DIFaddFL{). The decrease in coverage for the last 2Kb window is due to a tendency of the LLM to introduce large jumps in TMRCA in this window. Bottom row: the posterior variance, averaged over 1000 independent pivot pairs, as a function of position in the prediction frame and mutation rate (where $\mu$ stands for the species' mutation rate in }\stdpopsim\DIFaddFL{). As mutational information accrues, the posteriors become more precise; and lose precision towards the boundaries of the prediction frame where local context has been clipped.
}}
\label{fig:posterior-calibration}
\end{figure}

%DIF > %% Interpolation grid
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{figures/figS11_interpolation_grid.png}
\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Out-of-sample evaluation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Interpretation and limitations }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{the broad model on }%DIFDELCMD < \stdpopsim %%%
\DIFdelFL{v0.3}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\cxtkit$ by testing various recombination to mutation ratios (top left)}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Each panel shows inferred marginal coalescence distributions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The ratios have been chosen in such a way that most of stdpopsim v0.2 simulations, with the exception of some outliers, fall into this rectangle }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{dashed}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{top right}\DIFaddendFL )\DIFdelbeginFL \DIFdelFL{against true distributions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. The middle heatmaps show MSE and KLD values of $\cxtkit$ inferences }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{shaded}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{uncalibrated}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{over the entire grid, while the bottom heatmaps show mutation-rate-calibrated metrics}\DIFaddendFL . %DIF < probing the model's \textbf{capacity} - its ability to distinguish scenarios from context alone. 
\DIFdelbeginFL \DIFdelFL{We simulated data }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{For four representative examples }\DIFaddendFL from the \DIFdelbeginFL \DIFdelFL{novel species / demographic histories added in }%DIFDELCMD < \stdpopsim %%%
\DIFdelFL{v0.3}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{corners of this grid}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{aggregating pairwise inferences }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{see Figure~\ref{fig:bias-mutcoal}.}}
\label{fig:interp_corr}
\end{figure}

%DIF > %% Representative TMRCA examples from corners of interpolation grid
\begin{figure}[h!]
\centering
...
\end{subfigure}
\caption{\DIFaddFL{Interpretation and limitations of $\cxtkit$ zoomed }\DIFaddendFL into \DIFdelbeginFL \DIFdelFL{kernel density }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{four representative cases from the corners of the interpolation grid (Figure~\ref{fig:interp_corr}), starting from top left to bottom right, with MSE and KLD indicated in the subtitles. These }\DIFaddendFL plots \DIFaddbeginFL \DIFaddFL{show the TMRCA along the sequence, while both right sides show the marginal distribution, true and inferred, respectively}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{All results }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The left four panels (}\textbf{\DIFaddFL{a}}\DIFaddFL{) }\DIFaddendFL are \DIFdelbeginFL \DIFdelFL{from simulations previously unseen during training }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{uncalibrated and the right four }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{Top-Row: }%DIFDELCMD < \cxtkit%%%
\DIFdelFL{; Middle-Row: }\texttt{\DIFdelFL{Singer+Polegon}}%DIFAUXCMD
\DIFdelFL{; Bottom-Row: \smcpp}\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{b}}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{after calibration}\DIFaddendFL .}
\DIFdelbeginFL %DIFDELCMD < \label{fig:margdist_stdpop3}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < %%%
\DIFdelendFL %DIF > %% Fig S10: Cross coalescence
\DIFaddbeginFL \begin{figure}
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.9\textwidth]{./figures_v2/posterior_calibration.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The approximate posterior distributions sampled from by }%DIFDELCMD < \cxtkit %%%
\DIFdelFL{give well-calibrated estimates of uncertainty for predicted TMRCAs, across a range of scenarios. Top row: the proportion of true TMRCAs that fall within credibility intervals of a given width (e.g. a given proportion of posterior mass), as a function of position in the 1Mb prediction frame. Credibility intervals are calculated from quantiles of 100 samples from the approximate posterior for each pivot pair, and the proportion covering the true TMRCAs are calculated over 1000 independent pivot pairs (e.g. from distinct genealogical simulations). The expected proportions (for an exact posterior) are shown as dashed lines. Empirically, the approximate posteriors are asymptotically well-calibrated, although slightly over-concentrated for larger interval widths (likely due to the time discretization used by }%DIFDELCMD < \cxtkit%%%
\DIFdelFL{). The decrease in coverage for the last 2Kb window is due to a tendency of the LLM to introduce large jumps in TMRCA in this window. Bottom row: the posterior variance, averaged over 1000 independent pivot pairs, as a function of position in the prediction frame and mutation rate (where $\mu$ stands for the species' mutation rate in }%DIFDELCMD < \stdpopsim%%%
\DIFdelFL{). As mutational information accrues, the posteriors become more precise; and lose precision towards the boundaries of the prediction frame where local context has been clipped.
}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:posterior-calibration}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/figS10_cross_coalescence.png}
...
\DIFaddendFL \caption{Inference of \DIFaddbeginFL \DIFaddFL{coalescent-time landscapes in the Ag1000G }\emph{\DIFaddFL{A.~gambiae}} \DIFaddFL{dataset
...
For these countries, we infer }\DIFaddendFL coalescent times \DIFdelbeginFL \DIFdelFL{from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for
}\DIFaddendFL 25 focal \DIFaddbeginFL \DIFaddFL{haplotype }\DIFaddendFL pairs \DIFdelbeginFL \DIFdelFL{of British }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{per population; for Ghana, we analyze five diploid }\DIFaddendFL individuals\DIFdelbeginFL \DIFdelFL{of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{.
Light-blue curves indicate per-pair coalescent-time trajectories inferred by }\cxtkit\DIFaddFL{,
with }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{1000 Genomes project}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{mean trend shown in darker blue}\DIFaddendFL .
The \DIFdelbeginFL \DIFdelFL{top-left }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{lower }\DIFaddendFL panel shows \DIFdelbeginFL \DIFdelFL{chromosome~2, and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{genome-wide coalescent-time landscapes across }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{top-right panel shows }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{entirety of
}\DIFaddendFL chromosome~\DIFdelbeginFL \DIFdelFL{6. The LCT region (bottom-left) and HLA region (bottom-right) are zoomed respectively}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2L for Uganda samples}\DIFaddendFL .
\DIFdelbeginFL \DIFdelFL{Each pivot pair has been inferred 15 times to get a reliable mean, median }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Missing data density is shown beneath both the regional }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{variance estimates}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{genome-wide panels
as a genome browser track}\DIFaddendFL .
\DIFdelbeginFL \DIFdelFL{Additionally, light blue lines show the respective inferences of the 25 pairs.}\DIFdelendFL }
\DIFdelbeginFL %DIFDELCMD < \label{fig:chr2_6_corr}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:anopheles_main}
\DIFaddendFL \end{figure}

\DIFdelbegin %DIFDELCMD < \begin{figure}[t!]
%DIFDELCMD < \includegraphics[width=\textwidth]{./figures_revision/figure_7.png}
%DIFDELCMD < %%%
\DIFdelendFL %DIF > %% Fig S11: Mosquito full comparison
\DIFaddbeginFL \begin{figure}
\centering
\includegraphics[width=\textwidth]{figures/figS9_mosquito_comparison.png}
\DIFaddendFL \caption{Inference of coalescent-time landscapes in the Ag1000G \emph{A.~gambiae} dataset
...
with estimates from \cxtkit \DIFdelbeginFL \DIFdelFL{\ }\DIFdelendFL (left),
...
%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < %%%
\DIFdelendFL %DIF > %% TMRCA vs recombination
\DIFaddbeginFL \begin{figure}
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.9\textwidth]{./figures_revision/figure_S9.png}
%DIFDELCMD < \caption{%%%
\DIFdelFL{Interpretation and limitations of $\cxtkit$ by testing various recombination to mutation ratios top left. The ratios have been chosen in such way, that most of stdpopsim v0.2 with the exception of some outliers fall into this rectanlge (top right). The middle heatmaps, show MSE and KLD values of $\cxtkit$ inferences (uncalibrated) over the entire grid - while bottom heatmaps show mutation rate calibrated metrics.}%DIFDELCMD < \MBLOCKRIGHTBRACE
%DIFDELCMD < \label{fig:interp_corr}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.99\textwidth]{./figures_revision/figure_S10.png}
%DIFDELCMD < \caption{%%%
\DIFdelFL{Interpretation and limitations of $\cxtkit$ zoomed into four cases of the Supplementary Figures \ref{fig:interp_corr}. The figures are chosen to be representative for each of the corners of the heatmap of Figure \ref{fig:interp_corr}, starting from top left to bottom right, with MSE and KLD indicated in the subtitles . These plots themselves show the TMRCA along the sequence, while both right sides show the marginal distribution, true and inferred, respectively. The left four panels are uncalibrated and the right four after calibration.}%DIFDELCMD < \MBLOCKRIGHTBRACE
%DIFDELCMD < \label{fig:interp2_corr}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.9\textwidth]{./figures_revision/tmrca_recom.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/tmrca_recom.png}
