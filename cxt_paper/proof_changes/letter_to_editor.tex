...
We reformatted the manuscript to PNAS requests and made the following changes to the text:
The scientific content is fully preserved to was just done to fit the page limit.
...
Some MSE values were recomputed after providing now a single script which reproduces the entire paper. This lead to the identification of a minor bookkeeping error in the seeds of the post-hoc correction approach; no qualitative conclusion is affected. Consolidating the pipeline into a single script also introduced minor seed-level differences that propagate to the Singer+Polegon and SMC++ numbers, though the changes are negligible.

The same seed fix also affects the bias-correction step applied to the empirical mosquito TMRCA estimates (Figure~6 and the corresponding in-text summary statistics for the \emph{In(2L)a} inversion regions). The corrected random-number-generator state now advances independently across genomic blocks rather than resetting to a fixed value per block, yielding slightly higher absolute TMRCA values (roughly 20\% in low-TMRCA regions) with tighter cross-population standard deviations. The qualitative pattern (outside $<$ core $<$ inversion interior) is unchanged, and all conclusions regarding the inversion's genealogical signal remain the same.
...
Typos suggest by reviewer were fixed and acknowledgments updated
