...
parser.add_argument(
    ...
    default="/sietch_colab/data_share/cxt/experiments/coverage-constant/",
)
parser.add_argument(
...
demogr = stdpopsim.PiecewiseConstantSize(20000)
...
for m in [0.5, 1, 2]:
    ...
    if not os.path.exists(cache_file) or not os.path.exists(stats_file) or args.overwrite_cache:
        ...
        for epoch in range(num_epoch):
            ...
            job_list = [simulate_parallel.remote(s, m * base_m, base_r) for s in seed_array]
            ...
            mut_rate = np.repeat(m * base_m, mut_count.size)
